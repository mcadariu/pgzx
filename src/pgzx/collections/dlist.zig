...
pub fn DList(comptime T: type, comptime node_field: std.meta.FieldEnum(T)) type {
    return struct {
        ...
        //
        pub inline fn appendFromDList(self: *Self, other: *pg.c.dlist_head) void {
        ...
        pub inline fn appendToDList(self: *const Self, other: *pg.c.dlist_head) void {
        ...
        pub inline fn rawList(self: *Self) *pg.c.dlist_head {
            return &self.list;
        }
        ...
        pub inline fn isEmpty(self: *const Self) bool {
            return pg.c.dlist_is_empty(&self.list);
        }
        ...
        pub inline fn headNode(self: *const Self) *T {
            ...
            const node = pg.c.dlist_head_node(@constCast(&self.list));
        ...
        pub inline fn tailNode(self: *const Self) *T {
            ...
            const node = pg.c.dlist_tail_node(@constCast(&self.list));
            return descr.nodeParentPtr(node.?);
        }

        pub inline fn pushHead(self: *Self, node: *T) void {
            pg.c.dlist_push_head(&self.list, descr.nodePtr(node));
        }

        pub inline fn pushTail(self: *Self, node: *T) void {
            pg.c.dlist_push_tail(&self.list, descr.nodePtr(node));
        }
        ...
        pub inline fn popHead(self: *Self) *T {
            ...
            return descr.nodeParentPtr(pg.c.dlist_pop_head_node(&self.list));
        }
        ...
        pub inline fn popTail(self: *Self) *T {
            ...
            const tail = pg.c.dlist_tail_node(&self.list);
            pg.c.dlist_delete(tail);
            return descr.nodeParentPtr(tail.?);
        }

        pub inline fn moveHead(self: *Self, node: *T) void {
            pg.c.dlist_move_head(&self.list, descr.nodePtr(node));
        }

        pub inline fn moveTail(self: *Self, node: *T) void {
            pg.c.dlist_move_tail(&self.list, descr.nodePtr(node));
        }

        pub inline fn nextNode(self: *const Self, node: *T) *T {
            return descr.nodeParentPtr(pg.c.dlist_next_node(&self.list, descr.nodePtr(node)));
        }

        pub inline fn prevNode(self: *const Self, node: *T) *T {
            return descr.nodeParentPtr(pg.c.dlist_prev_node(&self.list, descr.nodePtr(node)));
        }
        ...
        pub inline fn insertAfter(node: *T, new_node: *T) void {
            pg.c.dlist_insert_after(descr.nodePtr(node), descr.nodePtr(new_node));
        }

        pub inline fn insertBefore(node: *T, new_node: *T) void {
            pg.c.dlist_insert_before(descr.nodePtr(node), descr.nodePtr(new_node));
        }

        pub inline fn delete(node: *T) void {
            pg.c.dlist_delete(descr.nodePtr(node));
        }

        pub inline fn deleteThorougly(node: *T) void {
            pg.c.dlist_delete_thoroughly(descr.nodePtr(node));
        }

        pub inline fn isDetached(node: *T) bool {
            return pg.c.dlist_is_detached(descr.nodePtr(node));
        }
    };
}

fn appendDList(to: *pg.c.dlist_head, from: *pg.c.dlist_head) void {
    if (pg.c.dlist_is_empty(from)) {
        return;
    }
...
fn DListIter(comptime T: type, comptime node_field: std.meta.FieldEnum(T)) type {
    return struct {
        ...
        iter: pg.c.dlist_iter,

        pub fn init(list: *const pg.c.dlist_head) Self {
            const end = &list.head;
            return .{
                .iter = .{
                    .end = @constCast(end),
...
pub fn DListDescr(comptime T: type, comptime node_field: std.meta.FieldEnum(T)) type {
    return struct {
        ...
        pub inline fn nodePtr(v: *T) *pg.c.dlist_node {
            return &@field(v, node);
        }

        pub inline fn nodeParentPtr(n: *pg.c.dlist_node) *T {
            return @fieldParentPtr(node, n);
        }

        pub inline fn optNodeParentPtr(n: ?*pg.c.dlist_node) ?*T {
